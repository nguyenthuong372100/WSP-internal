...
class HrPayslip(models.Model):
    ...
    def action_done(self):
        ...
        for payslip in self:
            if payslip.status == 'transfer_payment':
                payslip.status = 'done'
                _logger.info(f"Payslip {payslip.id} marked as Done by {self.env.user.name}")
    ...
    def action_revert_generated(self):
        ...
        for payslip in self:
            if payslip.status == 'employee_confirm':
                payslip.status = 'generated'
                _logger.info(f"Payslip {payslip.id} reverted to Generated by {self.env.user.name}")
    ...
    def action_revert_transfer_payment(self):
        ...
        for payslip in self:
            if payslip.status == 'transfer_payment':
                payslip.status = 'employee_confirm'
                _logger.info(f"Payslip {payslip.id} reverted to Employee Confirm by {self.env.user.name}")
