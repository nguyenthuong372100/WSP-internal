...
class HrEmployee(models.Model):
    ...
    payslip_ids = fields.One2many('hr.payslip', 'employee_id', string="Payslips", ondelete="cascade")
    ...
    def action_generate_payslip(self):
        ...
        for employee in self:
            ...
            payslip._compute_total_salary()
            payslip._compute_additional_fields()
            _logger.info(f"Payslip generated for {employee.name} for the month starting {payslip.date_from}")
...
class AttendanceTimesheetDetails(models.TransientModel):
    ...
    timesheet_html = fields.Html(string="Timesheet Details", compute="_compute_timesheet_html", readonly=True)
    approved = fields.Boolean(string="Approved", compute='_compute_approved', readonly=True)
